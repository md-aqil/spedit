...
const PlanCard = ({ title, description, price, priceInterval, isHighlighted, features }) => {
  return (
    <article className={`flex flex-col self-stretch px-6 pt-7 pb-${isHighlighted ? '28' : '11'} my-auto ${isHighlighted ? 'bg-neutral-800' : 'bg-white'} rounded-xl ${isHighlighted ? 'shadow-lg' : 'border border-solid border-stone-300'} min-h-[${isHighlighted ? '546' : '489'}px] min-w-[240px] w-[300px] max-md:px-5 ${isHighlighted ? 'max-md:pb-24' : ''}`}>
      ...
      <ul className="flex flex-col mt-7 w-full space-y-2">
        {features.map((feature, index) => (
          <li key={index} className="flex gap-3 items-center w-full">
            <div className={`flex gap-1.5 items-center self-stretch p-1.5 my-auto rounded-md ${isHighlighted ? 'bg-lime-500' : 'bg-neutral-800'} h-[18px] min-h-[18px] w-[18px]`}>
              <img loading="lazy" src={`http://b.io/ext_${index + 11}-`} alt="" className="object-contain w-2 aspect-square" />
            </div>
