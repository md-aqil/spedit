    ...
    <>
      ...
      >
        ...
        <header className="flex gap-10 justify-between items-center max-w-full whitespace-nowrap">
          <div className="flex flex-col items-start self-stretch my-auto">
            <div className="flex gap-2.5 items-start h-[43px]">
              <Image loading="lazy" src={userIcon} alt={`${name}'s avatar`} className="object-contain shrink-0 w-10 rounded-full aspect-square" />
              <div className="flex flex-col py-1">
                ...
                <div className="flex gap-1.5 items-center self-start mt-1.5 text-xs text-neutral-500">
                  <Image loading="lazy" src={JoinedUsers} alt="Rating" className="object-contain shrink-0 gap-0 self-stretch my-auto w-12 aspect-[2.67]" />
                  <span className="self-stretch my-auto">{rating}</span>
                </div>
              </div>
            </div>
          </div>
          <div className="overflow-hidden self-stretch px-2 py-1 my-auto text-xs text-center text-lime-500 rounded bg-neutral-800">Mock</div>
        </header>
        <div className="flex gap-6 justify-between mt-6">
          <div className="flex flex-col">
            <span className="text-xs text-neutral-500">Net Copy Amount</span>
            <span className="mt-2 text-xl font-bold text-black">{netCopyAmount.toLocaleString()}</span>
          </div>
          <div className="flex flex-col">
        ...
        <div className="flex gap-6 justify-between mt-6 w-full text-sm font-bold text-center">
          <button className="btn btn-danger-outline max-md:text-sm" onClick={(event) => handleStopClick(event)}>
            Stop Mock Copy
          </button>
          ...
          >
            <Link className=" !text-black" href="/copy-trading/strategy-description?name=copyStepOpen">
              Copy
            </Link>
          </button>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-gray-800 p-6 rounded-lg shadow-lg w-96">
            ...
            <div className="flex justify-end gap-2">
              <button onClick={() => setIsPopupVisible(false)} className="px-4 py-2 border border-gray-300 text-white rounded hover:bg-gray-700">
                Cancel
              </button>
              <button onClick={handleConfirmStop} className="px-4 py-2 bg-[#9BEC00] text-black rounded hover:bg-lime-600">
                Proceed
              </button>
            </div>
          </div>
      ...
      {showNotification && <div className="fixed bottom-4 right-4 bg-green-500 text-white py-2 px-4 rounded-lg">Successfully stopped copying!</div>}
    </>
  );
}
