...
function StrategyItem({ index, name, rating, avatar, profit, profitPercentage, roi, amount }) {
  ...
  return (
    <div
      id={index === 0 ? "lastTourStepId" : undefined}
    ...
    >
      ...
      {paused && <div className="stop-label">Paused (06:51 PM)</div>}

      {stopped && <div className="stop-label">Stoped {stoppedTime && `(${stoppedTime})`}</div>}
      ...
      <div className="flex w-full  items-center gap-10 st-top">
        <div className="w-full flex-wrap gap-10 justify-between items-center flex st-wrap">
          <div className="flex gap-3 items-start self-stretch my-auto whitespace-nowrap">
            <img loading="lazy" src={avatar} alt={`${name}'s avatar`} className="object-contain shrink-0 rounded-xl aspect-square w-[72px]" />
          ...
          <div className="flex gap-20">
            <div className="flex gap-1.5 justify-center items-center self-stretch py-2 my-auto text-base font-bold whitespace-nowrap rounded text-stone-950">
              <Image loading="lazy" src={Arrowup} alt="" className="object-contain shrink-0 w-3.5 aspect-square fill-lime-500" />
          ...
          <div className="flex gap-20  justify-between">
            ...
            <div className="gap-2 self-stretch p-2 my-auto text-sm font-semibold text-stone-950">₹{amount.toFixed(2)}</div>
          </div>

          {/* Heart Icon */}
          <Image loading="lazy" src={heartFullIcon} alt="Heart icon" className="object-contain shrink-0 self-stretch my-auto aspect-square w-[18px] hidden-xs" />
        </div>
        <div>
          {/* Action Buttons */}
          <div className="flex gap-4 items-center self-stretch my-auto text-sm font-bold text-center min-w-[240px] stia">
            ...
            <button onClick={handleStopCopyClick} className="gap-1 self-stretch px-2 py-3 my-auto text-white bg-pink-700 rounded min-h-[36px] w-[116px]">
              Stop Copy
            </button>
          </div>
        </div>
      ...
      {isModalOpen && (
        <ConfirmationModal
          ...
          message={actionType === "pause" ? "Are you sure you want to pause copying this strategy?" : "Are you sure you want to stop copying this strategy?"}
