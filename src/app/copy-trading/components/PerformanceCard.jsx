...
import StrategyCopyParameters2 from "./StargegyCopyParams2";
...
const PerformanceCard = ({
  ...
  return (
    <div className="flex flex-col justify-center items-center py-10 bg-white rounded-lg shadow-sm w-full px-10 PerformanceCard">
      ...
      {showParameters && <strategyCopyParameters2 showConfirmTour={showConfirmTour} showParameters={showParameters} />}
      {showParametersa ? (
        <strategyCopyParameters />
      ) : (
        <div className="flex flex-col w-full">
          ...
          <div className="flex gap-5 justify-between items-end mt-9 max-w-full text-sm leading-none text-center whitespace-nowrap roi-copy">
            ...
            <div className="flex flex-col per-act">
              <button id="copybuttonTour" className="px-7 py-5 btn mt-4 text-stone-950 w-[150px]" onClick={showParameters ? handleCopyClick : handleCopyClickAgain}>
                Copy
              </button>
              <button onClick={handleCopyClickAgain} className="btn btn-outline px-10 py-10 mt-4">
                Mock
              </button>
            </div>
          </div>
